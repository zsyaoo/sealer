...
func (c NativeDockerChecker) Check(cluster *v1.Cluster, phase string) error {
		...
		for _, ip := range ipList {
			...
			go func(ip string) {
				...
				if !c.SealerContainer {
					message = append(message, fmt.Sprintf("docker already installed in this host : %s. please uninstall docker", ip))
				}
			}(ip)
		}
		wg.Wait()
		if len(message) > 0 {
			return fmt.Errorf("docker check failed. %s", message)
		}
		return nil
	}
	...
	if phase == PhaseLiteBuild {
		...
		if err != nil {
			logger.Info("container is not installed lite build docker check success!")
			return nil
		}
